...
if sheet.pages.shape[0] > 1:  # True:
    # TODO: Extracting the note from each page:
    # TODO: Saving it in a list of lists
    #  Each sub-list contains chains of 24 notes or fewer, the code should be in chain process dir.
    #  CORRECTION: I WANT TO DIVIDE IT INTO CHAINS SO THAT THE LAST
    ...
    while not list(g.Event.read().split(" "))[-1] == str(sheet.pages.shape[0]):
        ...
        if turn_over_page(my_image, my_sound, name, frequency):
            ...
            sheet.next_page()
            # TODO: Change the windows, so they will be visible side by side here
            sheet.show()
